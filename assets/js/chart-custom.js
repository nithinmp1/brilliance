...
<tr class="hide">
 ...
 $('.table-add').on('click', 'i', () => {
   ...
   lastTR = $tableID.find('tbody tr').last();
   // lastTR.attr("id", parseInt(lastTRId) + 1);

   var URL = $('.table-add').attr("addDataUrl");
   var allTD = lastTR.children("td");
   let inputArray = {}
    allTD.each(function(index, element) {
      if ($(element).attr('columnname') !== undefined ) {
        inputArray[$(element).attr('columnname')] = $(element).text(); 
      }
    });
 ...
 $tableID.on('click', '.table-remove', function () {
    var URL = $(this).attr("deleteDataUrl");
    let inputArray = {}
    inputArray['id'] = $(this).attr("id"); 
    var jsonString = JSON.stringify(inputArray);
